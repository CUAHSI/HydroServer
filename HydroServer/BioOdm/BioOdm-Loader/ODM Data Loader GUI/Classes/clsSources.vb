...
#Region " File Field Constants "
    ...
    Public Const file_Sources_SourceFile As String = "sourcefile"                'R
    ...
    Protected Overrides Function ValidateTable(ByVal connect As SqlClient.SqlConnection, ByVal trans As System.Data.SqlClient.SqlTransaction) As Data.DataTable
        ...
        Try
            ...
            For i = 0 To (fileRows.Length - 1)
                ...
                'SourceFile
                If (m_ViewTable.Columns.IndexOf(file_Sources_SourceFile) >= 0) Then
                    If (fileRow.Item(file_Sources_SourceFile).ToString <> "") Then
                        Dim temp As String
                        temp = System.Text.RegularExpressions.Regex.Replace(fileRow.Item(file_Sources_SourceFile), "[\t\r\v\f\n]", "")

                        If System.Text.RegularExpressions.Regex.Matches(temp, "[\t\r\v\f\n]").Count() = 0 Then
                            tempRow.Item(db_fld_SourceFile) = temp
                        Else
                            Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & file_Sources_SourceFile & " contains invalid characters.")
                        End If
                    Else
                        Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & file_Sources_SourceFile & " cannot be NULL.")
                    End If
                Else
                    Throw New Exception("Unable to find the " & file_Sources_SourceFile & " column.")
                End If
