...
#Region " DataValues "
    ...
    Public Const db_fld_TaxaID As String = "TaxaID" 'O Integer -> TaxaID of the observation.  Linked to Taxonomy.TaxaID
...
#Region " Taxa "
    ' Taxa
    Public Const db_tbl_Taxa As String = "Taxonomy" 'Table Name
    Public Const db_fld_TTaxaID As String = "TaxaID" 'M Integer: Primary Key -> Unique ID for each Taxa entry
    Public Const db_fld_Family As String = "Family" 'O String: MAX -> Family
    Public Const db_fld_Genus As String = "Genus" 'O String: MAX -> Genus
    Public Const db_fld_Species As String = "Species" 'O String: MAX -> Species
    Public Const db_fld_TSN As String = "TSN" 'M String: MAX -> Taxonomic Serial Number assigned by the Integrated Taxonomic Information System
    Public Const db_fld_TaxaComments As String = "TaxaComments" 'O String: MAX -> Comments about the taxonomy description
#End Region
...
#Region " Sources "
    ...
    Public Const db_fld_SourceFile As String = "SourceFile" 'M String: 500 -> Text filename of the original data
...
#End Region
#Region " Taxa Columns "
    Public Const file_DataValues_TaxaID As String = "taxaid"                                'O
    Public Const file_DataValues_TSN As String = "tsn"                                'O
#End Region
...
#Region " Source Columns "
    ...
    Public Const file_DataValues_SourceFile As String = "sourcefile"                        'A
#End Region
...
#End Region
    ...
    Protected Overrides Function ValidateTable(ByVal connect As SqlClient.SqlConnection, ByVal trans As System.Data.SqlClient.SqlTransaction) As Data.DataTable
        ...
        Dim Sites, Variables, OffsetTypes, CensorCodes, Qualifiers, Methods, Sources, Samples, Taxa, QualityControlLevels, LabMethods, ISOMetadata, Units As New DataTable
        Dim i As Integer
        ...
        Try
            ...
            Taxa = m_Connection.OpenTable(connect, trans, db_tbl_Taxa, "SELECT * FROM " & db_tbl_Taxa)
            ...
            For i = 0 To (fileRows.Length - 1)
                ...
                If (m_ViewTable.Columns.IndexOf(file_DataValues_SourceID) >= 0) AndAlso (fileRow.Item(file_DataValues_SourceID).ToString <> "") Then
                    ...
                    End If
                ElseIf (m_ViewTable.Columns.IndexOf(file_DataValues_SourceFile) >= 0) Then
                    'They are trying to load DataValues using a SourceFile and they are not loading Sources
                    If (fileRow.Item(file_DataValues_SourceFile).ToString <> "") Then
                        Dim filename As String
                        filename = System.Text.RegularExpressions.Regex.Replace(fileRow.Item(file_DataValues_SourceFile), "[\t\r\v\f\n]", "")

                        If System.Text.RegularExpressions.Regex.Matches(filename, "[\t\r\v\f\n]").Count() = 0 Then
                            CVRows = Sources.Select(db_fld_SourceFile & " = '" & Replace(filename, "'", "''") & "'")
                            If (CVRows.Length > 0) Then
                                tempRow.Item(db_fld_SourceID) = Val(CVRows(CVRows.Length - 1).Item(db_fld_SSourceID))
                            Else
                                Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & "Unable to find the specified " & file_DataValues_SourceFile & " in the Sources table.")
                            End If
                        Else
                            Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & file_DataValues_SourceFile & " contains invalid characters.")
                        End If
                    Else
                        Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & file_DataValues_SourceFile & " cannot be NULL.")
                    End If
                ...
                Else
                    ...
                    Throw New Exception("Unable to load Source information.  You must specify either a SourceID or SourceLink or all of the required fields from the Sources table.")
                End If
                ...
                'TaxaID
                If (m_ViewTable.Columns.IndexOf(file_DataValues_TaxaID) >= 0) AndAlso (fileRow.Item(file_DataValues_TaxaID).ToString <> "") Then
                    'They are trying to load DataValues using a TaxaID and they are not loading Taxa
                    If IsNumeric(fileRow.Item(file_DataValues_TaxaID)) Then
                        CVRows = Taxa.Select(db_fld_TaxaID & " = '" & Val(fileRow.Item(file_DataValues_TaxaID)) & "'")
                        If (CVRows.Length > 0) Then
                            tempRow.Item(db_fld_TaxaID) = fileRow.Item(file_DataValues_TaxaID)
                        Else
                            Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & "Unable to find the specified " & file_DataValues_TaxaID & " in the Taxonomy table.")
                        End If
                    Else
                        Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & file_DataValues_TaxaID & " must be an integer.")
                    End If
                ElseIf (m_ViewTable.Columns.IndexOf(file_DataValues_TSN) >= 0) AndAlso (fileRow.Item(file_DataValues_TSN).ToString <> "") Then
                    'They are trying to load DataValues with the TSN of a taxa, so check first to see if the taxa already exists in the Taxonomy table
                    CVRows = Taxa.Select(db_fld_TSN & " = '" & Val(fileRow.Item(file_DataValues_TSN)) & "'")
                    If (CVRows.Length > 0) Then
                        'The Taxa already exists in the Taxonomy table
                        tempRow.Item(db_fld_TaxaID) = Val(CVRows(CVRows.Length - 1).Item(db_fld_TTaxaID))
                    Else
                        Throw New Exception("ROW # " & (m_ViewTable.Rows.IndexOf(fileRow) + 1) & ": " & "Unable to find Valid Taxonomy information.")
                    End If
                End If
        ...
        Catch ex As Exception
            ...
            If Not (CensorCodes Is Nothing) Then
                CensorCodes.Clear()
            End If
            Throw New ExitError("DataValues.ValidateTable(connect, trans)")
        End Try
