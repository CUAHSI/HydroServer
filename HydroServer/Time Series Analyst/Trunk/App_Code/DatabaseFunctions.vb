...
#Region " Sites "
    ...
    Public Const db_fld_SeriesId As String = "SeriesID" 'O String: 500 -> Comments related to the site
#End Region
...
#End Region
    ...
    ''' <summary>
    ''' Gets a StringDictionary containing Database_Name:Connection_String from the CapabilitiesDatabase database
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GetConnections() As NameValueCollection
        Try
            Dim Connections As New NameValueCollection()
            Dim data As DataTable = OpenHISNodeTable("SELECT * FROM ODMDatabases")
            Dim Key As String
            Dim Value As String
            For Each row As DataRow In data.Rows
                ...
                Value = "Data Source=" & row.Item("ServerAddress").ToString & _
    ...
    Public Shared Function GetODMVersion(ByVal dbname As String) As String
        Try
            Dim Table As New DataTable
            Dim ConnStr As String = GetConnectionString(dbname) 'Session("Database"))
            If Not (ConnStr = "") Then
                Dim testConn As New SqlClient.SqlConnection(ConnStr)
                testConn.Open()
        ...
        Catch ex As Exception
            Throw New Exception("Error Occured in DatabaseFunctions.GetODMVersion" & vbCrLf & ex.Message)
        End Try
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GetConnectionString(Optional ByVal dbName As String = "") As String
        Try
            Dim connectionString As String

            Dim Connections As NameValueCollection = GetConnections()

            If (dbName = "") Or (Connections(dbName) = "") Then
                If (ConfigurationManager.ConnectionStrings.Item(dbName) Is Nothing) Then
                    connectionString = Connections.Item(0)
                Else
                    connectionString = ConfigurationManager.ConnectionStrings.Item(dbName).ConnectionString
                End If
            Else
                connectionString = Connections.Item(dbName)
            End If
    ...
    ''' <summary>
    ''' Returns a dataTable of the query data from the HIS Node Database.
    ''' </summary>
    ...
    ''' <remarks></remarks>
    Public Shared Function OpenHISNodeTable(ByVal sqlQuery As String) As DataTable
        'create a flow table
        ...
        Try
            Dim connectionString As String = ConfigurationManager.ConnectionStrings.Item("CapabilitiesDatabase").ConnectionString
    ...
    ''' <summary>
    ''' Returns a dataTable of the query data from the selected odm database.
    ''' </summary>
    ''' <param name="sqlQuery">sql Query to retreive the data with</param>
    ''' <param name="DBName">The name of the database to connect to</param>
    ''' <returns>The dataTable of data retreived from the database using SqlQuery.</returns>
    ''' <remarks></remarks>
    Public Shared Function OpenODMTable(ByVal sqlQuery As String, Optional ByVal DBName As String = "") As DataTable
        'create a flow table
        Dim table As New DataTable() 'the table of data to return
        Dim dataAdapter As New SqlClient.SqlDataAdapter  'the dataAdapter to fill the table
        Dim objSqlConnection As SqlConnection
    ...
    ''' <summary>
    ''' Executes a non-query on the database to see if the command can be run, without altering the data
    ''' </summary>
    ''' <param name="objSqlCommand">the sql command to excute</param>
    ''' <param name="DBName">the database name to query</param>
    ''' <remarks></remarks>
    Public Shared Sub ExecuteNonQuery(ByVal objSqlCommand As SqlCommand, Optional ByVal DBName As String = "")
        Try
            Dim connectionString As String = GetConnectionString(DBName)
    ...
    ''' <param name="DBName">the database name to query</param>
    ''' <remarks></remarks>
    Public Shared Sub ExecuteNonQuery(ByVal strSQL As String, Optional ByVal DBName As String = "")
        Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function FormatDateForQueryFromDB(ByVal d As Date) As String
        Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function FormatParameter(ByVal val As String) As String
        Try
            '    'Formats the date according to what type of data base connection is selected
            '    ' Sql puts quotes and oledb puts # around the date
            '    'Inputs:  d -> the date to format
